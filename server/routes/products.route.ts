...
export const productsRouter = (imageStorage: ImageStorage) => {
  const router = Router();
  ...
  router.get("/product-url", async (req, res) => {
    try {
      const params = req.query as GetProductUrlParams;
      const result = validateImage(params);
      if (result.error) {
        return res
          .status(StatusCode.ClientErrorBadRequest)
          .send(result.error.message);
      ...
      // }
      const url = await imageStorage.getUrl(params.imageName);
      res.json(url);
    } catch (e) {
      console.error(e);
      return res.status(StatusCode.ServerErrorInternal).send("internal error");
